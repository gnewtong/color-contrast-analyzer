...
export function ColorRampEditor({ colorRamps, onColorRampsChange, onDuplicateRamp }: ColorRampEditorProps) {
  ...
  const originalStopNames = useRef<{ [key: string]: string }>({});
  ...
  // When user focuses the stop name input, store the original value
  const handleStopNameFocus = (rampId: string, stopIndex: number, currentName: string) => {
    const key = `${rampId}-${stopIndex}`;
    originalStopNames.current[key] = currentName;
  };

  const handleStopChange = (rampId: string, stopIndex: number, field: 'name' | 'hex', value: string) => {
    const key = `${rampId}-${stopIndex}`;
    setEditingStates((prev: any) => ({
      ...prev,
      [key]: {
        ...prev[key],
        [field]: value
  ...
  const handleStopBlur = (rampId: string, stopIndex: number, field: 'name' | 'hex') => {
    ...
    if (field === 'name') {
      ...
      if (ramp) {
        ...
        const duplicate = ramp.stops.some((stop, idx) =>
          idx !== stopIndex && stop.name.trim().toLowerCase() === lowerValue
        );
        if (duplicate) {
          // Revert to original value
          setEditingStates((prev: any) => {
            const newState = { ...prev };
            newState[key] = { ...newState[key], name: originalStopNames.current[key] ?? ramp.stops[stopIndex].name };
            return newState;
          });
          // Also update the actual ramp stop to the original value
          updateColorStop(rampId, stopIndex, 'name', originalStopNames.current[key] ?? ramp.stops[stopIndex].name);
          // Clear editing state after revert
          setTimeout(() => {
            setEditingStates((prev: any) => {
              const newState = { ...prev };
              delete newState[key];
  ...
  const toggleLockedStop = (rampId: string, stopIndex: number) => {
    ...
    onColorRampsChange(
      colorRamps.map(r =>
        r.id === rampId ? { ...r, lockedStops: Array.from(lockedStops) } : r
      )
    );
  };
  ...
  const isStopLocked = (rampId: string, stopIndex: number) => {
    const ramp = colorRamps.find(r => r.id === rampId);
    return ramp?.lockedStops?.includes(stopIndex) || false;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {colorRamps.map((ramp) => (
        <Card key={ramp.id} className="w-full p-0">
          ...
          <CardContent>
            <div className="space-y-3">
              {ramp.stops.map((stop, stopIndex) => {
                ...
                const stopKey = `${ramp.id}-${stopIndex}`;
                return (
                  <div key={stopIndex} className="flex items-end gap-3">
                    <div
                      className="w-8 h-8 rounded border border-border relative"
                    ...
                    <div className="flex items-center gap-2 flex-1">
                      <div className="flex flex-col gap-1">
                        ...
                        <Input
                          ...
                          value={editingStates[stopKey]?.name ?? stop.name}
                          onChange={(e) => handleStopChange(ramp.id, stopIndex, 'name', e.target.value)}
                          onFocus={() => handleStopNameFocus(ramp.id, stopIndex, stop.name)}
                          onBlur={() => handleStopBlur(ramp.id, stopIndex, 'name')}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault();
                              handleStopBlur(ramp.id, stopIndex, 'name');
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => handleStopChange(ramp.id, stopIndex, 'hex', sanitizeHexInput(e.target.value))}
