...
export function ColorRampGenerator({ onGenerateRamp, existingRamp, onUndo, canUndo }: ColorRampGeneratorProps) {
  // Single source of truth for configuration
  const [config, setConfig] = useState<RampConfig>({
    referenceColors: existingRamp ? existingRamp.stops.map(s => s.hex) : ['#FFCC00'],
    targetRatios: existingRamp ? existingRamp.stops.map((stop, index) => {
  ...
  const [lockedRampPositions, setLockedRampPositions] = useState<Set<number>>(existingRamp?.lockedStops || new Set());
              ...
              {Array.from({ length: config.numReferenceColors }, (_, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    className={lockedColors.has(i) ? 'bg-blue-50' : ''}
                  >
