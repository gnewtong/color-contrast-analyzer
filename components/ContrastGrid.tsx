...
export function ContrastGrid({ 
  ...
  const handleCellClick = (xColor: string, yColor: string, xRampId: string, yRampId: string, xStopIndex: number, yStopIndex: number) => {
    // Prevent clicking on cells that would edit the same stop on both axes
    if (xRampId === yRampId && xStopIndex === yStopIndex) {
      return;
    }
    ...
    const locked: string[] = [];
    if (xRamp?.lockedStops?.includes(xStopIndex)) {
      locked.push(xRamp.stops[xStopIndex]?.name || 'X');
    }
    if (yRamp?.lockedStops?.includes(yStopIndex)) {
  ...
  const applyAdjustment = (option: ContrastAdjustmentOption) => {
    ...
    const yRamp = colorRamps.find(r => r.id === selectedCell.yRampId);
    const locked: string[] = [];
    if (option.xColor && xRamp?.lockedStops?.includes(selectedCell.xStopIndex)) {
      locked.push(xRamp.stops[selectedCell.xStopIndex]?.name || 'X');
    }
    if (option.yColor && yRamp?.lockedStops?.includes(selectedCell.yStopIndex)) {
  ...
  const toggleLockedStop = (rampId: string, stopIndex: number) => {
    ...
    const updatedRamps = colorRamps.map(ramp => {
      if (ramp.id === rampId) {
        const lockedStopsArray = Array.from(ramp.lockedStops || []);
        const lockedStops = new Set(lockedStopsArray);
        if (lockedStops.has(stopIndex)) {
          lockedStops.delete(stopIndex);
        } else {
          lockedStops.add(stopIndex);
        }
        return { ...ramp, lockedStops: Array.from(lockedStops) };
      }
      return ramp;
    });
    onColorRampsChange(updatedRamps);
  ...
  const addStopBefore = (rampId: string, stopIndex: number) => {
    ...
    const updatedRamps = colorRamps.map(r => {
      if (r.id === rampId) {
        ...
        // Update lockedStops indices to account for the new stop
        const updatedLockedStops: number[] = [];
        if (r.lockedStops) {
          for (const lockedIndex of r.lockedStops) {
            if (lockedIndex >= stopIndex) {
              // Shift locked indices that come after or at the insertion point
              updatedLockedStops.push(lockedIndex + 1);
            } else {
              // Keep locked indices that come before the insertion point
              updatedLockedStops.push(lockedIndex);
            }
          }
        }
        
        return { ...r, stops: newStops, lockedStops: updatedLockedStops };
      }
      return r;
    });
    ...
    const updatedRamps = colorRamps.map(r => {
      if (r.id === rampId) {
        ...
        if (r.lockedStops) {
          for (const lockedIndex of r.lockedStops) {
            if (lockedIndex > stopIndex) {
              // Shift locked indices that come after the insertion point
              updatedLockedStops.push(lockedIndex + 1);
            } else {
              // Keep locked indices that come at or before the insertion point
              updatedLockedStops.push(lockedIndex);
            }
          }
        }
  ...
  const EditableColorSwatch = React.memo(({ 
    ...
    // Track the original stop name when editing begins
    const originalNameRef = useRef(stop.name);
    ...
    const handleNameFocus = () => {
      ...
      originalNameRef.current = stop.name;
    };
    ...
    const handleNameBlur = () => {
      ...
      if (nameValue !== stop.name) {
        // Check for duplicate name in the same ramp
        const ramp = colorRamps.find(r => r.id === rampId);
        if (ramp) {
          const lowerValue = nameValue.trim().toLowerCase();
          const duplicate = ramp.stops.some((s, idx) => idx !== stopIndex && s.name.trim().toLowerCase() === lowerValue);
          if (duplicate) {
            // Revert to original value
            setNameValue(originalNameRef.current);
            updateColorStop(rampId, stopIndex, 'name', originalNameRef.current);
            return;
          }
        }
    ...
    return (
      <div
                        className={`relative border border-border flex items-end justify-center ${padding} overflow-hidden aspect-square`}
        style={{ 
  ...
  const AddStopButtonCell = ({ 
    ...
    return (
      ...
      >
        {showButton && (
          <div className="rounded-full p-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 shadow-sm w-6 h-6 flex items-center justify-center transition-all hover:scale-110">
  ...
  return (
    <div ref={containerRef} className="w-full space-y-4" style={{ scrollBehavior: 'auto' }}>
      ...
      <div className="relative w-full overflow-x-auto">
        ...
        >
          {/* Top-left empty cell */}
                          <div className="bg-background border border-border aspect-square" aria-label="Empty cell - top left corner" />
          ...
          {xRamp.stops.map((stop, index) => {
            ...
            const isLocked = xRamp.lockedStops?.includes(index) || false;
          ...
          {yRamp.stops.map((rowStop, rowIndex) => {
            ...
            const isLocked = yRamp.lockedStops?.includes(rowIndex) || false;
            ...
            return (
              <React.Fragment key={`row-${rowStop.name}`}>
                ...
                {xRamp.stops.map((colStop) => {
                  ...
                  // Check if this cell represents the same stop on both axes
                  const isSameStop = yRamp.id === xRamp.id && 
                    yRamp.stops.findIndex(s => s.name === rowStop.name) === xRamp.stops.findIndex(s => s.name === colStop.name);
                  
                  return (
                    <div
                      key={`cell-${rowStop.name}-${colStop.name}`}
                      className={`border border-border flex items-center justify-center ${padding} aspect-square transition-all ${
                        isSameStop 
                          ? 'opacity-60 bg-transparent' 
                          : 'cursor-pointer hover:ring-2 hover:ring-blue-300'
                      }`}
                      style={{ 
                        backgroundColor: isSameStop ? 'transparent' : bgColor
                      }}
                      role="gridcell"
                    ...
                    >
                      <div className={`text-center ${isSameStop ? 'text-foreground' : 'text-black'} ${fontSizes.ratio}`}>
